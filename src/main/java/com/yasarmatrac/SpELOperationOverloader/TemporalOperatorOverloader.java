...
public class TemporalOperatorOverloader implements OperatorOverloader {

    @Override
    public boolean overridesOperation(Operation operation, Object leftOperand, Object rightOperand) throws EvaluationException {
        if (leftOperand instanceof Temporal && rightOperand instanceof Duration
                && (operation == Operation.SUBTRACT || operation == Operation.ADD)) {
            return true;
        } else if (leftOperand instanceof Temporal && rightOperand instanceof Temporal && operation == Operation.SUBTRACT) {
            return true;
        }
        return false;
    }
